...
fn main() {
    ...
    for (i, instruction) in program.iter().enumerate() {
        ...
        let program_mod = match instruction.typ {
            InstructionType::Acc => continue,
            InstructionType::Jmp => {
                let mut program_mod = program.clone();
                program_mod[i].typ = InstructionType::Nop;
                program_mod
            }
            InstructionType::Nop => {
                let mut program_mod = program.clone();
                program_mod[i].typ = InstructionType::Jmp;
                program_mod
            }
        };
        loop {
            ...
            let instruction = program_mod[machine.pc as usize];
